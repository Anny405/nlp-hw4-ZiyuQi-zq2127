...
def do_train(args, model, train_dataloader, save_dir="./out"):
    ...
    global device  # 使用 main 里设置好的 device
...
def create_augmented_dataloader(args, dataset):
    ...
    num_aug = 5000
    if len(train_dataset) < num_aug:
        num_aug = len(train_dataset)
    aug_subset = train_dataset.shuffle(seed=42).select(range(num_aug))

    aug_transformed = aug_subset.map(custom_transform, load_from_cache_file=False)

    augmented_train = datasets.concatenate_datasets([train_dataset, aug_transformed])
    augmented_tokenized = augmented_train.map(tokenize_function, batched=True)
